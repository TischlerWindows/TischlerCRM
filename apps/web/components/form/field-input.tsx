...
export function FieldInput({
  ...
  // Admin users may override auto-number values on existing records (edit mode).
  // useAuth must be called before any conditional returns (Rules of Hooks).
  ...
  // Admin can edit an AutoNumber field that already has a value (i.e. edit mode, not create).
  const canOverrideAutoNumber = isAdminUser && fieldDef.type === 'AutoNumber' && Boolean(value);
  const isReadOnly =
    (fieldDef.readOnly ||
        ...
        inputElement = (
          <div className="relative">
            <Input
              ...
              className={cn(commonProps.className, 'border-amber-400 pr-8')}
            />
            <span
              className="absolute right-2 top-1/2 -translate-y-1/2 text-amber-500"
              title="Admin override — changing this value may affect numbering sequences"
            >
