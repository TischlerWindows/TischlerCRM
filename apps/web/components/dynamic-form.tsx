...
export default function DynamicForm({
    ...
    layout.tabs.forEach(tab => {
      tab.regions.forEach(region => {
        region.panels.forEach(panel => {
          ...
          const rawType = (field as any).type || object.fields.find(f => f.apiName === field.fieldApiName)?.type;
  ...
  const getFieldDef = (apiName: string, pageField?: import('@/lib/schema').PageField): FieldDef | undefined => {
    ...
    if (pageField && pageField.type && pageField.label) {
      const { column, order, presentation: _p, colSpan: _cs, rowSpan: _rs, ...fieldProps } =
        pageField;
      return {
  ...
  const validateForm = (): boolean => {
    ...
    layout.tabs.forEach((tab) => {
      tab.regions.forEach((region) => {
        region.panels.forEach((panel) => {
          panel.fields.forEach((field) => {
            const fieldDef = getFieldDef(field.fieldApiName, field as any);
            if (!fieldDef) return;
            ...
            const isRequired = fieldDef.required || field.behavior === 'required';
            if (isRequired && !isAutoGenerated) {
              if (value === undefined || value === null || value === '') {
                newErrors[field.fieldApiName] = `${fieldDef.label} is required`;
              }
            ...
            if (value !== undefined && value !== null && value !== '') {
              switch (fieldDef.type) {
                case 'Email':
                  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
                    newErrors[field.fieldApiName] = 'Invalid email format';
                  }
                  break;
                case 'URL':
                  try {
                    ...
                    newErrors[field.fieldApiName] = 'Invalid URL format';
                  }
                  break;
                case 'Phone':
                  if (!/^[\d\s\-\+\(\)]+$/.test(value)) {
                    newErrors[field.fieldApiName] = 'Invalid phone format';
                  }
                  break;
                case 'Number':
                case 'Currency':
                case 'Percent':
                  if (isNaN(Number(value))) {
                    newErrors[field.fieldApiName] = 'Must be a valid number';
                  }
                  break;
              }
            }
            ...
            if (value !== undefined && value !== null && value !== '') {
              if (fieldDef.min !== undefined && Number(value) < fieldDef.min) {
                newErrors[field.fieldApiName] = `Value must be at least ${fieldDef.min}`;
              }
              if (fieldDef.max !== undefined && Number(value) > fieldDef.max) {
                newErrors[field.fieldApiName] = `Value must be at most ${fieldDef.max}`;
              }
            }
            ...
            if (typeof value === 'string') {
              if (fieldDef.minLength !== undefined && value.length < fieldDef.minLength) {
                newErrors[field.fieldApiName] = `Must be at least ${fieldDef.minLength} characters`;
              }
              if (fieldDef.maxLength !== undefined && value.length > fieldDef.maxLength) {
                newErrors[field.fieldApiName] = `Must be at most ${fieldDef.maxLength} characters`;
              }
            }
          });
        });
  ...
  // Validate only the fields in a specific panel (used for wizard "Next" navigation)
  const validateSection = (panel: import('@/lib/schema').LayoutPanel): boolean => {
    const newErrors: Record<string, string> = {};
      ...
      if (value !== undefined && value !== null && value !== '') {
        switch (fieldDef.type) {
          case 'Email':
            ...
            break;
          case 'URL':
            try { new URL(value); } catch { newErrors[field.fieldApiName] = 'Invalid URL format'; }
            break;
          case 'Phone':
            if (!/^[\d\s\-\+\(\)]+$/.test(value)) {
              newErrors[field.fieldApiName] = 'Invalid phone format';
  ...
  // Compute wizard sections: flatten all visible panels across all tabs (create mode only)
  const wizardSections = (() => {
    if (layoutType !== 'create') return [];
    const allSections: { section: import('@/lib/schema').LayoutPanel; tabLabel: string }[] = [];
    layout.tabs.forEach((tab) => {
      tab.regions.forEach((region) => {
        region.panels
          .sort((a, b) => a.order - b.order)
          .forEach((panel) => {
            const isVisible = evaluateVisibility((region as any).visibleIf, formData, visibilityCtx);
            ...
            if (isVisible && (region as any).showInTemplate !== false && !secFx?.hidden && !panel.hidden) {
  ...
  const renderField = (fieldDef: FieldDef, stretch?: boolean, layoutField?: PageField) => {
  ...
  // Helper to render a single panel's content (shared between wizard and normal modes)
  const renderSectionContent = (panel: import('@/lib/schema').LayoutPanel) => {
          ...
          >
            {columnArrays.map((columnEntries, colIndex) => (
              <div key={`col-${colIndex}`} className="flex flex-col gap-4">
                {columnEntries.map((entry) => renderField(entry.fieldDef, false, entry.pageField))}
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    ...
    <form onKeyDown={handleFormKeyDown} className="flex flex-col h-full">
      ...
      {!showReview && !isWizardMode && (
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {(() => {
            // Build canvas items from regions and their widgets
            type CanvasItem =
              | { kind: 'region'; region: import('@/lib/schema').LayoutSection }
            ...
            const filtered = canvasItems.filter((item) => {
              ...
              const isRegionVisible = evaluateVisibility((region as any).visibleIf, formData, visibilityCtx);
                    ...
                    return (
                      <div
                        ...
                        style={gridItemStyle({
                          ...
                          gridColumnSpan: (g as any).gridColumnSpan ?? TAB_GRID_COLUMNS,
                      ...
                      return (
                        <div
                          ...
                          style={gridItemStyle({
                            ...
                            gridColumnSpan: region.gridColumnSpan ?? TAB_GRID_COLUMNS,
                        ...
                        >
                          ...
                          >
                            <div className="text-left">
                              <h3 className="text-lg font-semibold text-gray-900">{panelItem.label}</h3>
                ...
                .map((panelItem) => {
                  ...
                  for (let ci = 0; ci < panelItem.columns; ci++) {
                    columnArrays[ci] = panelItem.fields
                      .filter((f) => ((f as any).column ?? (f.order % panelItem.columns)) === ci)
                      .sort((a, b) => a.order - b.order)
                  ...
                  return (
                    <div key={panelItem.id} className="bg-white rounded-lg border border-gray-200">
                      <div className="p-4 bg-gray-100 rounded-t-lg">
                        <h3 className="text-lg font-semibold text-gray-900">{panelItem.label}</h3>
                        {(panelItem as any).description ? (
                          <p className="text-xs text-gray-500 mt-1 font-normal">{(panelItem as any).description}</p>
                        ) : null}
                      </div>
                      <div className="p-4">
                        <div
                          className={cn(
                            ...
                            panelItem.columns === 2 && 'grid-cols-1 md:grid-cols-2',
                            panelItem.columns === 3 && 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3',
                            panelItem.columns === 4 && 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'
                          )}
                        >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm text-gray-500 mt-1">
                        ...
                        {tl.tabs?.reduce((acc: number, tab: any) => acc + (tab.regions?.length || 0), 0) || 0} sections
                      </div>
                    </div>
                  </button>
                ))}
