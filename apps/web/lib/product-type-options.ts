export function getOptionsForType(t: string): string[] {
  ...
  if (lo === 'pivot' || lo === 'outswing pivot' || lo.includes('convert pivot')) {
    return ['Maco Instinct Motorized Locks', 'Arched'];
  }
  ...
  // Window folding types (e.g. 'Inswing Folding Window', 'Outswing Folding Window') — no Arched
  ...
  if (lo.includes('inswing') && (lo.includes(' gd') || lo.includes(' dd') || lo.includes('house door'))) {
    return ['72mm Thick Sash', '90mm Thick Sash', 'KFV RH', 'Siegenia RH', 'Threshold #6', '#6C', 'ADA', 'Arched'];
  }
  if (lo.includes('outswing') && (lo.includes(' gd') || lo.includes(' dd') || lo.includes('house door'))) {
    return ['72mm Thick Sash', '90mm Thick Sash', 'KFV RH', 'Siegenia RH', 'Threshold #7', '#8', 'ADA', 'Arched'];
  }
  if (lo.includes('offset simulated') || lo.includes('offset french simulated')) {
    return ['72mm Thick Sash', '84mm Thick Sash', 'Corrosion Resistance RH', 'Titan RH', 'Arched'];
  }
  if (lo.includes('simulated dh') || lo.includes('simulated double hung')) {
    return ['72mm Thick Sash', '90mm Thick Sash', 'Corrosion Resistance RH', 'Titan RH', 'Arched'];
  }
  if (lo.includes('single hung') || lo.includes('double hung') || lo.includes('triple hung')) {
    return ['59mm Thick Sash', '72mm Thick Sash', '82mm Thick Sash', '90mm Thick Sash', 'Vent Locks', 'Arched'];
  }
  if (lo.includes('direct glaze')) {
    return ['72mm Thick Sash', '90mm Thick Sash', 'Threshold to match', 'Arched'];
  }
  if (lo.includes('fixed with sash')) {
    return ['59mm Thick Sash', '72mm Thick Sash', '82mm Thick Sash', '90mm Thick Sash', 'Threshold to match', 'Arched'];
  }
  ...
  return ['72mm Thick Sash', '90mm Thick Sash', 'Arched'];
}
