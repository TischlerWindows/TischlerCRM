...
export default function ProposalPdfViewerClient() {
  ...
  useEffect(() => {
    setPageWidth(Math.min(window.innerWidth - 48, 816));
  }, []);

  useEffect(() => {
    if (!templateId || !summaryId) { setLoading(false); return; }
    ...
    fetch(`${apiBase}/proposal-pdf/render`, {
      ...
      .then((buf) => { setPdfBytes(new Uint8Array(buf)); setLoading(false); })
      .catch((err: unknown) => {
        setError(err instanceof Error ? err.message : 'Failed to load PDF');
        setLoading(false);
      });
  }, [templateId, summaryId]);
  ...
  return (
    <div className="flex h-dvh flex-col bg-gray-700">
      <div className="flex shrink-0 items-center gap-3 border-b border-gray-600 bg-gray-800 px-4 py-2 text-white text-sm">
        ...
        {numPages > 0 && (
          <div className="ml-auto flex items-center gap-2">
            ...
            <span className="tabular-nums">Page {pageNumber} of {numPages}</span>
            <button
      ...
      {/* externalLinkTarget="_blank" makes react-pdf force target=_blank on every link */}
