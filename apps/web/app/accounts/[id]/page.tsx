...
export default function AccountDetailPage() {
  ...
  const getLayoutForAccount = (): PageLayout | null => {
    ...
    if (recordType?.pageLayoutId) {
      const pageLayout = accountObject.pageLayouts?.find(l => l.id === recordType.pageLayoutId);
      if (pageLayout) return pageLayout;
    }

    // Default to first layout
    return accountObject.pageLayouts?.[0] || null;
  };
  ...
  const getFieldDef = (apiName: string): FieldDef | undefined => {
    return accountObject?.fields.find(f => f.apiName === apiName);
  };
                            ...
                            return (
                              ...
                              >
                                {rowFields.map((layoutField) => {
                                  ...
                                  const value = account[layoutField.apiName] || account[layoutField.apiName.replace(/^[^_]+__/, '')];
