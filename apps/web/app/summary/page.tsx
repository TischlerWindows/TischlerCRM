...
export default function SummaryPage() {
  ...
  const [showType4, setShowType4] = useState(true);
    ...
    setEditingSummary({
      ...
      rows: editingSummary.rows.map(r => {
        ...
        if (field === 'widthMM') {
          ...
          if (widthMM) {
            // BG calculation
            const roundedWidth = Math.ceil(widthMM / 100) * 100; // ROUNDUP(E6,-2)
            const bgCalc = roundedWidth < 1000 ? (1000 / 1000 * 100) : (roundedWidth / 1000 * 100);
            const bg = Math.ceil(bgCalc); // ROUNDUP
            
            // BI: =IF(E6<1000,3,ROUND((E6-2*50)/335,0))
            const bi = widthMM < 1000 ? 3 : Math.round((widthMM - 2 * 50) / 335);
            
            // BJ calculation
            const bj = 11 * bi; // (3+8) * BI
            
            // BK calculation
            const bk = widthMM <= 1000 ? 72 : (widthMM <= 2000 ? 102 : 132);
            
            // Final calculation: ROUNDUP(SUM(BG+BH+BJ+BK)*1.362161, 0)
            // BH is always 0 since BF is 0
            updatedRow.shadeBoxesNoSideTrimUnit = Math.ceil((bg + bj + bk) * 1.362161).toString();
            ...
            // Auto-calculate Shade Boxes with Trim (based on width)
            const heightMM = parseFloat(updatedRow.heightMM);
            const roundedWidthTrim = Math.ceil(widthMM / 100) * 100;
            const brCalc = roundedWidthTrim < 1000 ? (1000 / 1000 * 100) : (roundedWidthTrim / 1000 * 100);
            const br = Math.ceil(brCalc);
            const roundedHeight = Math.ceil(heightMM / 100) * 100;
            const bsCalc = roundedHeight < 1000 ? (1000 / 1000 * 260) : (roundedHeight / 1000 * 260);
        ...
        <div className="px-6 py-6">
          ...
          <div className="mb-6">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search summaries by name or description..."
                value={searchTerm}
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-[95vw] max-h-[95vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Summary Name</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingSummary({ ...editingSummary, description: e.target.value })}
                    maxLength={400}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-indigo-500"
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-100">
                      ...
                      <tr>
                        ...
                        <th className="px-0.5 py-1 text-left text-xs font-medium text-gray-700 w-16">Width (MM)</th>
                        <th className="px-0.5 py-1 text-left text-xs font-medium text-gray-700 w-16">Height (MM)</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {editingSummary.rows.map((row) => (
                        <tr key={row.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-0.5 py-1 align-top">
                            <CellInput rowId={row.id} field="type" value={row.type} onChange={(v) => updateRow(row.id, 'type', v)} />
                          </td>
                          <td className="px-0.5 py-1 align-top">
                            <CellInput rowId={row.id} field="qty3" value={row.qty3} onChange={(v) => updateRow(row.id, 'qty3', v)} />
                          </td>
                          <td className="px-0.5 py-1 align-top">
                            <CellInput rowId={row.id} field="type2" value={row.type2} onChange={(v) => updateRow(row.id, 'type2', v)} />
                          </td>
                          {showType3 && (
                            <td className="px-0.5 py-1 align-top">
                          ...
                          {showType3 && (
                            <td className="px-0.5 py-1 align-top">
                              <CellInput rowId={row.id} field="type3" value={row.type3} onChange={(v) => updateRow(row.id, 'type3', v)} />
                            </td>
                          )}
                          {showType4 && (
                            <td className="px-0.5 py-1 align-top">
                          ...
                          {showType4 && (
                            <td className="px-0.5 py-1 align-top">
                              <CellInput rowId={row.id} field="type4" value={row.type4} onChange={(v) => updateRow(row.id, 'type4', v)} />
                            </td>
                          )}
                          <td className="px-0.5 py-1 align-top">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {editingSummary.doorRows.map((row) => (
                        <tr key={row.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-0.5 py-1 align-top">
                            <CellInput rowId={row.id} field="type" value={row.type} onChange={(v) => updateDoorRow(row.id, 'type', v)} />
                          </td>
                          <td className="px-0.5 py-1 align-top">
                            <CellInput rowId={row.id} field="qty3" value={row.qty3} onChange={(v) => updateDoorRow(row.id, 'qty3', v)} />
                          </td>
                          <td className="px-0.5 py-1 align-top">
                            <CellInput rowId={row.id} field="type2" value={row.type2} onChange={(v) => updateDoorRow(row.id, 'type2', v)} />
                          </td>
                          {showType3 && (
                            <td className="px-0.5 py-1 align-top">
                          ...
                          {showType3 && (
                            <td className="px-0.5 py-1 align-top">
                              <CellInput rowId={row.id} field="type3" value={row.type3} onChange={(v) => updateDoorRow(row.id, 'type3', v)} />
                            </td>
                          )}
                          {showType4 && (
                            <td className="px-0.5 py-1 align-top">
                          ...
                          {showType4 && (
                            <td className="px-0.5 py-1 align-top">
                              <CellInput rowId={row.id} field="type4" value={row.type4} onChange={(v) => updateDoorRow(row.id, 'type4', v)} />
                            </td>
                          )}
                          <td className="px-0.5 py-1 align-top">
