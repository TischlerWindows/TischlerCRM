...
export default function ProjectsPage() {
  ...
  const AVAILABLE_COLUMNS = useMemo(() => {
    if (!projectObject?.fields) {
      return [
        ...
        { id: 'expectedCompletion', label: 'Expected Completion', defaultVisible: true },
        { id: 'assignedTeam', label: 'Assigned Team', defaultVisible: true },
    ...
    return projectObject.fields.map((field, index) => {
      const cleanApiName = field.apiName.replace('Project__', '');
  ...
  const filteredProjects = projects.filter(project => {
    ...
    switch (sidebarFilter) {
      case 'recent':
        matchesSidebar = new Date(project.lastModifiedAt) >= thirtyDaysAgo;
        break;
      case 'created-by-me':
        matchesSidebar = project.createdBy === 'Development User';
        break;
      case 'favorites':
        matchesSidebar = (project as any).isFavorite === true;
        break;
      case 'all':
      default:
        matchesSidebar = true;
    ...
    if (sortColumn.includes('At') || sortColumn === 'startDate' || sortColumn === 'endDate') {
  ...
  if (!canAccess('Project', 'read')) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="w-16 h-16 text-red-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h2>
          <p className="text-gray-600 mb-6">You don&apos;t have permission to view Projects.</p>
        ...
        <div className={`${isMobile ? 'fixed top-[88px] bottom-0 w-44 z-30' : 'relative w-64'} left-0 bg-white border-r border-gray-200 ${isMobile ? 'p-4' : 'p-6'} overflow-y-auto flex-shrink-0 transition-transform duration-200 ease-in-out ${isMobile ? (mobileSidebarOpen ? 'translate-x-0 shadow-xl' : '-translate-x-full') : 'translate-x-0'}`}>
          ...
          <div className="space-y-6" style={isLandscapeMobile ? { transform: 'scale(0.85)', transformOrigin: 'top left' } : undefined}>
            ...
            <div className="pb-6 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600 ml-13">Track active installations and project execution</p>
            </div>
      ...
      <div className="px-3 md:px-6 py-6">
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search projects by number, name, or status..."
              value={searchTerm}
                ...
                {filteredProjects.map((project) => (
                  <tr key={project.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/projects/${project.id}`)}>
                      ...
                      return (
                        <td key={column.id} className="px-6 py-4 text-sm text-gray-900">
                          {column.id === 'projectNumber' ? (
                            ...
                            formatColumnValue(project, column.id)
                          )}
                        </td>
                      );
                    })}
                    ...
                    <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                      ...
                      >
                        <Star className={`w-5 h-5 ${project.isFavorite ? 'fill-yellow-400 text-yellow-400' : 'text-gray-400'}`} />
                      </button>
                    </td>
                  </tr>
                ))}
            ...
            <div className="p-6">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="text-sm text-gray-700">
                  <p className="mb-3">
                    To create new projects, you need to configure a page layout in the Page Editor first.
                  </p>
