...
export default function InstallationsPage() {
  ...
  const formatColumnValue = (installation: Installation, columnId: string) => {
    ...
    const schemaFieldForFormula = installationObject?.fields?.find(f => f.apiName === `Installation__${columnId}` || f.apiName === columnId);
    if (schemaFieldForFormula?.type === 'Formula' && schemaFieldForFormula.formulaExpr) {
      const computed = evaluateFormulaForRecord(schemaFieldForFormula.formulaExpr, installation as any, installationObject);
    ...
    const schemaLookupField = installationObject?.fields?.find(f => f.apiName === `Installation__${columnId}` || f.apiName === columnId);
  ...
  if (!canAccess('Installation', 'read')) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="w-16 h-16 text-red-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h2>
          <p className="text-gray-600 mb-6">You don&apos;t have permission to view Installations.</p>
          ...
          <div className="space-y-6" style={isLandscapeMobile ? { transform: 'scale(0.85)', transformOrigin: 'top left' } : undefined}>
            <div className="pb-6 border-b border-gray-200">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 bg-[#e8eaf6] rounded-lg flex items-center justify-center"><HardHat className="w-6 h-6 text-brand-navy" /></div>
