...
export default function InstallationDetailPage() {
  ...
  // Get the layout based on installation's pageLayoutId or recordTypeId
  const getLayoutForInstallation = () => {
    if (!installation || !installationObject) return null;

    if (installation.pageLayoutId) {
      const pageLayout = installationObject.pageLayouts?.find(l => l.id === installation.pageLayoutId);
      if (pageLayout) {
        return pageLayout;
      }
    }

    const recordTypeId = installation.recordTypeId;
    const recordType = recordTypeId
      ? installationObject.recordTypes?.find(rt => rt.id === recordTypeId)
      : installationObject.recordTypes?.[0];

    const pageLayoutId = recordType?.pageLayoutId;
    const pageLayout = pageLayoutId
      ? installationObject.pageLayouts?.find(l => l.id === pageLayoutId)
      : installationObject.pageLayouts?.[0];

    return pageLayout;
  };

  const pageLayout = getLayoutForInstallation();

  const getFieldsFromLayout = () => {
    if (!pageLayout || !installationObject) return [];
    ...
    return (installationObject.fields || []).filter(field => 
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {section.fields?.map((layoutField: any, fieldIndex: number) => {
                        ...
                        const fieldDef = installationObject?.fields?.find(
                          f => f.apiName === layoutField.apiName || f.apiName === normalizedFieldName
                        );
                        const value = installation[normalizedFieldName] || installation[layoutField.apiName];
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {displayFields.map((field, index) => (
                  <div key={index}>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      {formatFieldValue(installation[field.apiName], field.type) || '-'}
                    </dd>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-6 bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-900">{installation.createdBy}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-gray-400" />
              <span className="text-gray-500">Created:</span>
              <span className="text-gray-900">{installation.createdAt}</span>
            </div>
            <div className="flex items-center gap-2">
              <User className="w-4 h-4 text-gray-400" />
              <span className="text-gray-500">Modified by:</span>
              <span className="text-gray-900">{installation.lastModifiedBy}</span>
            </div>
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-gray-400" />
              <span className="text-gray-500">Modified:</span>
              <span className="text-gray-900">{installation.lastModifiedAt}</span>
            </div>
          </div>
        </div>
      </div>
