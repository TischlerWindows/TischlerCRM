...
export default function QuoteDetailPage() {
  ...
  // Get the layout based on quote's pageLayoutId or recordTypeId
  const getLayoutForQuote = () => {
    if (!quote || !quoteObject) return null;

    if (quote.pageLayoutId) {
      const pageLayout = quoteObject.pageLayouts?.find(l => l.id === quote.pageLayoutId);
      if (pageLayout) {
        return pageLayout;
      }
    }

    const recordTypeId = quote.recordTypeId;
    const recordType = recordTypeId
      ? quoteObject.recordTypes?.find(rt => rt.id === recordTypeId)
      : quoteObject.recordTypes?.[0];

    const pageLayoutId = recordType?.pageLayoutId;
    const pageLayout = pageLayoutId
      ? quoteObject.pageLayouts?.find(l => l.id === pageLayoutId)
      : quoteObject.pageLayouts?.[0];

    return pageLayout;
  };

  const pageLayout = getLayoutForQuote();

  const getFieldsFromLayout = () => {
    if (!pageLayout || !quoteObject) return [];
    ...
    return (quoteObject.fields || []).filter(field => 
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {section.fields?.map((layoutField: any, fieldIndex: number) => {
                        ...
                        const fieldDef = quoteObject?.fields?.find(
                          f => f.apiName === layoutField.apiName || f.apiName === normalizedFieldName
                        );
                        const value = quote[normalizedFieldName] || quote[layoutField.apiName];
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {displayFields.map((field, index) => (
                  <div key={index}>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      {formatFieldValue(quote[field.apiName], field.type) || '-'}
                    </dd>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-6 bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-900">{quote.createdBy}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-gray-400" />
              <span className="text-gray-500">Created:</span>
              <span className="text-gray-900">{quote.createdAt}</span>
            </div>
            <div className="flex items-center gap-2">
              <User className="w-4 h-4 text-gray-400" />
              <span className="text-gray-500">Modified by:</span>
              <span className="text-gray-900">{quote.lastModifiedBy}</span>
            </div>
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-gray-400" />
              <span className="text-gray-500">Modified:</span>
              <span className="text-gray-900">{quote.lastModifiedAt}</span>
            </div>
          </div>
        </div>
      </div>
