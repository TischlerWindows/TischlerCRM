...
export default function QuotesPage() {
  ...
  const canCreateQuote = canAccess('Quote', 'create');
  const canEditQuote = canAccess('Quote', 'edit');
  const canDeleteQuote = canAccess('Quote', 'delete');
  ...
  const formatColumnValue = (quote: Quote, columnId: string) => {
    ...
    const schemaFieldForFormula = quoteObject?.fields?.find(f => f.apiName === `Quote__${columnId}` || f.apiName === columnId);
    if (schemaFieldForFormula?.type === 'Formula' && schemaFieldForFormula.formulaExpr) {
      const computed = evaluateFormulaForRecord(schemaFieldForFormula.formulaExpr, quote as any, quoteObject);
    ...
    const schemaLookupField = quoteObject?.fields?.find(f => f.apiName === `Quote__${columnId}` || f.apiName === columnId);
  ...
  if (!canAccess('Quote', 'read')) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="w-16 h-16 text-red-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h2>
          <p className="text-gray-600 mb-6">You don&apos;t have permission to view Quotes.</p>
