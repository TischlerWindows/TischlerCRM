...
export default function ProductsPage() {
  ...
  const formatColumnValue = (product: Product, columnId: string) => {
    ...
    const schemaFieldForFormula = productObject?.fields?.find(f => f.apiName === `Product__${columnId}` || f.apiName === columnId);
    if (schemaFieldForFormula?.type === 'Formula' && schemaFieldForFormula.formulaExpr) {
      const computed = evaluateFormulaForRecord(schemaFieldForFormula.formulaExpr, product as any, productObject);
    ...
    const schemaFieldFinal = productObject?.fields?.find(f => f.apiName === `Product__${columnId}` || f.apiName === columnId);
  ...
  if (!canAccess('Product', 'read')) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="w-16 h-16 text-red-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h2>
          <p className="text-gray-600 mb-6">You don&apos;t have permission to view Products.</p>
