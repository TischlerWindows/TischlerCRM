  ...
  return (
    ...
    <div className="flex flex-1 bg-gray-50 h-screen relative">
        ...
        {isMobile && !mobileSidebarOpen && (
          <button onClick={() => setMobileSidebarOpen(true)} className="fixed left-0 top-1/2 -translate-y-1/2 z-30 bg-white border border-l-0 border-gray-100 rounded-r-md px-0.5 py-3 shadow-sm" aria-label="Open sidebar">
            <ChevronRight className="w-3.5 h-3.5 text-gray-500" />
          </button>
        )}
        {/* Sidebar */}
        <div className={`${isMobile ? 'fixed top-[88px] bottom-0 w-44 z-30' : 'relative w-64'} left-0 bg-white border-r border-gray-100 ${isMobile ? 'p-4' : 'p-6'} overflow-y-auto flex-shrink-0 transition-transform duration-200 ease-in-out ${isMobile ? (mobileSidebarOpen ? 'translate-x-0 shadow-xl' : '-translate-x-full') : 'translate-x-0'}`}>
          {/* Mobile close arrow */}
          {isMobile && (
