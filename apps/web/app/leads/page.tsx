...
export default function LeadsPage() {
  ...
  const formatColumnValue = (lead: Lead, columnId: string) => {
    ...
    let value = (lead as any)[columnId];

    // Formula fields: evaluate expression instead of showing raw value
    const schemaFieldForFormula = leadObject?.fields?.find(f => f.apiName === `Lead__${columnId}` || f.apiName === columnId);
    if (schemaFieldForFormula?.type === 'Formula' && schemaFieldForFormula.formulaExpr) {
      const computed = evaluateFormulaForRecord(schemaFieldForFormula.formulaExpr, lead as any, leadObject);
    ...
    const schemaLookupField = leadObject?.fields?.find(f => f.apiName === `Lead__${columnId}` || f.apiName === columnId);
  ...
  if (!canAccess('Lead', 'read')) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="w-16 h-16 text-red-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Access Denied</h2>
          <p className="text-gray-600 mb-6">You don&apos;t have permission to view Leads.</p>
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-gray-50 relative">
        ...
        <div className={`${isMobile ? 'fixed top-[88px] bottom-0 w-44 z-30' : 'relative w-64'} left-0 bg-white border-r border-gray-200 ${isMobile ? 'p-4' : 'p-6'} overflow-y-auto flex-shrink-0 transition-transform duration-200 ease-in-out ${isMobile ? (mobileSidebarOpen ? 'translate-x-0 shadow-xl' : '-translate-x-full') : 'translate-x-0'}`}>
          ...
          <div className="pb-6 border-b border-gray-200 mb-6" style={isLandscapeMobile ? { transform: 'scale(0.85)', transformOrigin: 'top left' } : undefined}>
