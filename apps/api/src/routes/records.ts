...
export async function recordRoutes(app: FastifyInstance) {
  ...
  app.post('/objects/:apiName/records', async (req, reply) => {
    ...
    // Also skip auto-generated number fields — these are populated by the app, not the user.
    const autoGeneratedFieldNames = new Set([
    ...
    const missingFields = requiredFields.filter((f) => {
      ...
      // Also check if any data key, after stripping its object prefix, matches
      // the DB field apiName (handles schema-prefixed keys like Account__accountName
      // matching against DB field 'accountName' even when DB stores 'name').
      const strippedKeyMatch = Object.keys(data).some((k) => {
        const stripped = k.replace(/^[A-Za-z]+__/, '');
        return stripped === f.apiName && data[k] !== undefined && data[k] !== null;
      });
      return (
